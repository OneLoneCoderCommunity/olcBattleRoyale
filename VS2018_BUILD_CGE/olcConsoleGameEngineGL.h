...
class olcConsoleGameEngineGL
{
	...
	{
		static olcConsoleGameEngineGL *cge;


		switch (uMsg)
		{
		case WM_CREATE:
			cge = static_cast<olcConsoleGameEngineGL*>(((LPCREATESTRUCT)lParam)->lpCreateParams);
...
public:
	void Start()
	{
		...
		// Star the thread
		thread t = thread(&olcConsoleGameEngineGL::GameThread, this);
...
protected:
	...
	protected: // Audio Engine =====================================================================
			...
			unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
		{
			...
			m_AudioThread = std::thread(&olcConsoleGameEngineGL::AudioThread, this);
		...
		{
			((olcConsoleGameEngineGL*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
		}
...
atomic<bool> olcConsoleGameEngineGL::m_bAtomActive(false);
condition_variable olcConsoleGameEngineGL::m_cvGameFinished;
mutex olcConsoleGameEngineGL::m_muxGame;